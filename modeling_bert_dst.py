...
from transformers.models.bert.modeling_bert import (BertModel, BertPreTrainedModel, BERT_START_DOCSTRING,
                                                    BERT_INPUTS_DOCSTRING, _CHECKPOINT_FOR_DOC, _CONFIG_FOR_DOC,
                                                    _TOKENIZER_FOR_DOC, QuestionAnsweringModelOutput)
...
class BertForDST(BertPreTrainedModel):
    def __init__(self, config):
        ...
        #for coreference
        self.refer_decoder = Decoder(config)
        ...
        self.add_module("class_" , nn.Linear(config.hidden_size + aux_dims, self.class_labels))
        self.add_module("refer_" , nn.Linear(config.hidden_size + aux_dims, len(self.slot_list) + 1))
    ...
    def forward(self,
        ...
        #Decorder learn the slot the correlations
        refer_hidden = self.refer_decoder(sequence_output, input_mask, slot_lookup)
        ...
        for ids , slot in enumerate(self.slot_list):
            ...
            class_logits = self.dropout_heads(getattr(self, 'class_' )(refer_hidden[:,ids,:]))
            ...
            refer_logits = self.dropout_heads(getattr(self, 'refer_' )(refer_hidden[:,ids,:]))
            ...
            if class_label_id is not None and start_pos is not None and end_pos is not None and refer_id is not None:
                ...
                if TRAINING:
                    # Low
                    class_loss_fct = LOWLoss()
                    token_loss_fct = CrossEntropyLoss(reduction='none', ignore_index=ignored_index)
                    refer_loss_fct = CrossEntropyLoss(reduction='none')
                    # w1_1,start_loss = token_loss_fct(start_logits, start_pos[slot])
                    # w1_2,end_loss = token_loss_fct(end_logits, end_pos[slot])
                    # token_loss = (torch.mul(w1_1,start_loss) + torch.mul(w1_2,end_loss)) / 2.0
                    ...
                    # w2,refer_loss = refer_loss_fct(refer_logits, refer_id[slot])
                    ...
                    w, class_loss = class_loss_fct(class_logits, class_label_id[slot])
                    ...
                    # refer_loss=refer_loss_fct.update(w2,refer_loss)
                    class_loss = class_loss_fct.update(w, class_loss)

                    if self.refer_index > -1:
                        per_example_loss_low = (self.class_loss_ratio) * class_loss + (
                                (1 - self.class_loss_ratio) / 2) * token_loss + (
                                                       (1 - self.class_loss_ratio) / 2) * refer_loss
                    else:
                        per_example_loss_low = self.class_loss_ratio * class_loss + (
                    ...
                    total_loss += per_example_loss_low.sum()
                else:
