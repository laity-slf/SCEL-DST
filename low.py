...
class LOWLoss(torch.nn.Module):
    def __init__(self, lamb=0.5, ignore_index: int =-100):
        super(LOWLoss, self).__init__()
        self.lamb = lamb  # higher lamb means more smoothness -> weights closer to 1
        self.ignore_index=ignore_index
        self.loss = torch.nn.CrossEntropyLoss(reduction='none',ignore_index=self.ignore_index)  # replace this with any loss with "reduction='none'"
    ...
    @staticmethod
    def update(weights, loss):

        # return torch.mean(torch.mul(loss, weights), dim=0)
        return torch.mul(loss, weights)
