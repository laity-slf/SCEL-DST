    ...
    with open(fp) as f:
        ...
        for pred in preds:
            ...
            if joint_gt_slot == joint_pd_slot:
                ...
                if no_match:
                    ...
                    total_correct = False
                    print("  [%s] Incorrect value (variant): %s (turn class: %s) | %s (turn class: %s)" % (
                ...
                val_correctness.append(0.0)
                total_correct = False
                print("  [%s] Incorrect value: %s (turn class: %s) | %s (turn class: %s)" % (
                    guid, joint_gt_slot, turn_gt_class, joint_pd_slot, turn_pd_class))
        ...
        for ct in range(len(class_types)):
            ...
            print(
                ...
                (class_types[ct], ct, recall, np.sum(class_correctness[ct]), len(class_correctness[ct]), precision, f1,
                 acc, c_tp[ct], c_tn[ct], c_fp[ct], c_fn[ct]))
        ...
        return np.asarray(total_correctness), np.asarray(val_correctness), np.asarray(class_correctness), np.asarray(
            pos_correctness), np.asarray(refer_correctness), np.asarray(confusion_matrix), c_tp, c_tn, c_fp, c_fn
...
if __name__ == "__main__":
    ...
    dataset = 'multiwoz21'
    dataset_config = f'dataset_config/{dataset}.json'
    ...
    out_path = '/Myhome/slf/work/科博杯/trippy-public/results'
    # fp = '/Myhome/slf/work/科博杯/trippy-public/results_refer_modelxiugai/pred_res.test.final.json'
    fp = os.path.join(out_path, 'pred_res.test.9624.json')
    if fp:
        print(fp)
        ...
        c_fn = {ct: 0 for ct in range(len(class_types))}
        for slot in slots:
            tot_cor, joint_val_cor, cls_cor, pos_cor, ref_cor, conf_mat, ctp, ctn, cfp, cfn = get_joint_slot_correctness(
                fp, class_types, label_maps,
            ...
            ss = slot.split('-')
            if ss[0] in res_slot:
                res_slot[ss[0]] *=  tot_cor
            else:
                res_slot[ss[0]] = tot_cor
            print(
                '%s: joint slot acc: %g, joint value acc: %g, turn class acc: %g, turn position acc: %g, turn referral acc: %g' %
                (slot, np.mean(tot_cor), np.mean(joint_val_cor), np.mean(cls_cor[-1]), np.mean(pos_cor),
                 np.mean(ref_cor)))
            res[slot] = {'joint slot acc': np.mean(joint_val_cor), 'slot gate acc': np.mean(cls_cor[-1]),
                         'span prediction acc': np.mean(pos_cor), 'refer acc': np.mean(ref_cor)}
        ...
        for ct in range(len(class_types)):
            ...
            print(
                "Performance for class '%s' (%s): Recall: %.2f (%d of %d), Precision: %.2f, F1: %.2f, Accuracy: %.2f (TP/TN/FP/FN: %d/%d/%d/%d)" %
                (class_types[ct], ct, recall, np.sum(cls_acc[ct]), len(cls_acc[ct]), precision, f1, acc, c_tp[ct],
                 c_tn[ct], c_fp[ct], c_fn[ct]))
    ...
    for k , v in res_slot.items():
        print(f'the Domain {k} acc is : {np.mean(v)}')

    col = []
    row = []
    for k, v in res.items():
        row.append(k)
        col.append([v['joint slot acc'], v['slot gate acc'], v['span prediction acc'], v['refer acc']])
    col = np.array(col)
    df = pd.DataFrame(data={'slot name':row,
                            'joint slot acc': col[:, 0],
                            'slot gate acc': col[:, 1],
                            'span prediction acc': col[:, 2],
                            'refer acc': col[:, 3]},
                      index=[i for i in range(len(row))])

    # out_path = '/Myhome/slf/work/科博杯/trippy-public/results_refer_best'
    df.to_csv(os.path.join(out_path, 'sce_DST.csv'), index=True)
