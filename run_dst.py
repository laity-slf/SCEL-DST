...
from model import UtteranceEncoding
...
ALL_MODELS += ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST
ALL_MODELS += ALBERT_PRETRAINED_MODEL_ARCHIVE_LIST

MODEL_CLASSES = {
    'bert': (BertConfig, BertForDST, BertTokenizer),
    'roberta': (RobertaConfig, RobertaForDST, RobertaTokenizer),
    'albert': (AlbertConfig, AlbertForDST, AlbertTokenizerFast)
}

from modeling_bert_dst import update
...
def train(args, train_dataset, features, model, tokenizer, processor, slot_lookup, continue_from_global_step=0):
    """ Train the model """
    ...
    model_list = list(model.named_parameters())
    model_decoder_list = model.refer_decoder.parameters()
    model_encoder = model_list[0:199] + model_list[199 + 46:]

    optimizer_grouped_parameters = [
        {'params': [p for n, p in model_encoder if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay},
        {'params': [p for n, p in model_encoder if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}
    ]

    # optimizer_bert = AdamW(optimizer_grouped_parameters, lr=args.learning_rate, eps=args.adam_epsilon)
    optimizer_bert = AdamW(optimizer_grouped_parameters, lr=args.bert_learning_rate, eps=args.adam_epsilon)

    scheduler_bert = get_linear_schedule_with_warmup(optimizer_bert, num_warmup_steps=num_warmup_steps,
                                                     num_training_steps=t_total)

    optimizer_attention = AdamW(model_decoder_list, lr=args.attention_learning_rate, eps=args.adam_epsilon)

    scheduler_attention = get_linear_schedule_with_warmup(optimizer_attention, num_warmup_steps=num_warmup_steps,
                                                          num_training_steps=t_total)
    ...
    if args.fp16:
        ...
        model, optimizer = amp.initialize(model, optimizer_bert, opt_level=args.fp16_opt_level)
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.fp16:
                with amp.scale_loss(loss, optimizer_bert) as scaled_loss:
                    scaled_loss.backward()
                torch.nn.utils.clip_grad_norm_(amp.master_params(optimizer_bert), args.max_grad_norm)
            else:
                loss.backward()
        ...
        if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
            results = evaluate(args, model_single_gpu, tokenizer, processor, slot_lookup, prefix=global_step)
...
def adv_train(args, train_dataset, features, model, tokenizer, processor, slot_lookup, continue_from_global_step=0):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()
...
def evaluate(args, model, tokenizer, processor, slot_lookup, prefix=""):
    update()
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        for slot in model.slot_list:
            ...
            for i, u in enumerate(updates):
                if u != 0:
                    if u != 2 and slot == 'restaurant-food':
                        print(1)
                    diag_state[slot][i] = u
...
def main():
    ...
    parser.add_argument("--attention_learning_rate", default=1e-4, type=float,
                        help="The initial learning rate for Adam.")
    ...
    parser.add_argument('--do_adversarial', action='store_true',
                        help="Whether to add adversarial.")
    ...
    # Fixed Bert
    slot_encoder = UtteranceEncoding.from_pretrained(args.model_name_or_path,
                                                     from_tf=bool('.ckpt' in args.model_name_or_path),
                                                     config=config)
    for p in slot_encoder.parameters():
        p.requires_grad = False

    slot_lookup = get_label_lookup_from_first_token(dst_slot_list, tokenizer, slot_encoder, device)
    ...
    config.dst_refer_attn_head = 4
    config.dst_num_self_attention_layer = 2
    config.dst_refer_attn_type = 'softmax'
    ...
    if args.do_train:
        ...
        if args.do_adversarial:
            global_step, tr_loss = adv_train(args, train_dataset, features, model, tokenizer, processor, slot_lookup,
                                             continue_from_global_step)
        else:
            global_step, tr_loss = train(args, train_dataset, features, model, tokenizer, processor, slot_lookup,
                                         continue_from_global_step)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for cItr, checkpoint in enumerate(checkpoints):
            ...
            # Evaluate
            result = evaluate(args, model, tokenizer, processor, slot_lookup, prefix=global_step)
